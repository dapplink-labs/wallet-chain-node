...
// newLineaClient init the eth client
func newLineaClients(conf *config.Config) ([]*lineaClient, error) {
	...
	var clients []*lineaClient
	for _, rpc := range conf.Fullnode.Linea.RPCs {
		client := &lineaClient{
			chainConfig:   chainConfig,
			confirmations: conf.Fullnode.Linea.Confirmations,
		}
		rpcURL := rpc.RPCURL
...
func newLocalLineaClient(network config.NetWorkType) *lineaClient {
...
func (client *lineaClient) blockNumber() *big.Int {
	now := time.Now().Unix()
	client.rw.RLock()
...
func (client *lineaClient) isContractAddress(address common.Address) bool {
	code, err := client.CodeAt(context.Background(), address, nil)
	return err == nil && len(code) > 0
}

func (client *lineaClient) GetLatestBlockHeight() (int64, error) {
