...
// newzksyncClient init the eth client
func newZksyncClients(conf *config.Config) ([]*zksyncClient, error) {
	...
	var clients []*zksyncClient
	for _, rpc := range conf.Fullnode.Zksync.RPCs {
		client := &zksyncClient{
			chainConfig:   chainConfig,
			confirmations: conf.Fullnode.Zksync.Confirmations,
		}
		rpcURL := rpc.RPCURL
...
func newLocalZksyncClient(network config.NetWorkType) *zksyncClient {
...
func (client *zksyncClient) blockNumber() *big.Int {
	now := time.Now().Unix()
	client.rw.RLock()
...
func (client *zksyncClient) isContractAddress(address common.Address) bool {
	code, err := client.CodeAt(context.Background(), address, nil)
	return err == nil && len(code) > 0
}

func (client *zksyncClient) GetLatestBlockHeight() (int64, error) {
