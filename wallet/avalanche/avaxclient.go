...
// newAvaxClient init the eth client
func newAvaxClients(conf *config.Config) ([]*avaxClient, error) {
	...
	var clients []*avaxClient
	for _, rpc := range conf.Fullnode.Avax.RPCs {
		client := &avaxClient{
			chainConfig:   chainConfig,
			confirmations: conf.Fullnode.Avax.Confirmations,
		}
		rpcURL := rpc.RPCURL
...
func newLocalAvaxClient(network config.NetWorkType) *avaxClient {
...
func (client *avaxClient) blockNumber() *big.Int {
	now := time.Now().Unix()
	client.rw.RLock()
...
func (client *avaxClient) isContractAddress(address common.Address) bool {
	code, err := client.CodeAt(context.Background(), address, nil)
	return err == nil && len(code) > 0
}

func (client *avaxClient) GetLatestBlockHeight() (int64, error) {
