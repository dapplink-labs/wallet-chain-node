...
// newbaseClient init the eth client
func newBaseClients(conf *config.Config) ([]*baseClient, error) {
	...
	var clients []*baseClient
	for _, rpc := range conf.Fullnode.Base.RPCs {
		client := &baseClient{
			chainConfig:   chainConfig,
			confirmations: conf.Fullnode.Base.Confirmations,
		}
		rpcURL := rpc.RPCURL
...
func newLocalBaseClient(network config.NetWorkType) *baseClient {
...
func (client *baseClient) blockNumber() *big.Int {
	now := time.Now().Unix()
	client.rw.RLock()
...
func (client *baseClient) isContractAddress(address common.Address) bool {
	code, err := client.CodeAt(context.Background(), address, nil)
	return err == nil && len(code) > 0
}

func (client *baseClient) GetLatestBlockHeight() (int64, error) {
