...
// newhecoClient init the eth client
func newHecoClients(conf *config.Config) ([]*hecoClient, error) {
	...
	var clients []*hecoClient
	for _, rpc := range conf.Fullnode.Heco.RPCs {
		client := &hecoClient{
			chainConfig:   chainConfig,
			confirmations: conf.Fullnode.Heco.Confirmations,
		}
		rpcURL := rpc.RPCURL
...
func newLocalHecoClient(network config.NetWorkType) *hecoClient {
...
func (client *hecoClient) blockNumber() *big.Int {
	now := time.Now().Unix()
	client.rw.RLock()
...
func (client *hecoClient) isContractAddress(address common.Address) bool {
	code, err := client.CodeAt(context.Background(), address, nil)
	return err == nil && len(code) > 0
}

func (client *hecoClient) GetLatestBlockHeight() (int64, error) {
