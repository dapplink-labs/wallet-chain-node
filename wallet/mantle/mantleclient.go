...
// newMantleClient init the eth client
func newMantleClients(conf *config.Config) ([]*mantleClient, error) {
	...
	var clients []*mantleClient
	for _, rpc := range conf.Fullnode.Mantle.RPCs {
		client := &mantleClient{
			chainConfig:   chainConfig,
			confirmations: conf.Fullnode.Mantle.Confirmations,
		}
		rpcURL := rpc.RPCURL
...
func newLocalMantleClient(network config.NetWorkType) *mantleClient {
...
func (client *mantleClient) blockNumber() *big.Int {
	now := time.Now().Unix()
	client.rw.RLock()
...
func (client *mantleClient) isContractAddress(address common.Address) bool {
	code, err := client.CodeAt(context.Background(), address, nil)
	return err == nil && len(code) > 0
}

func (client *mantleClient) GetLatestBlockHeight() (int64, error) {
