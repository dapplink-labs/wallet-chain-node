...
func NewChainAdaptor(conf *config.Config) (wallet.WalletAdaptor, error) {
	clients, err := newAdaClients(conf)
	if err != nil {
		return nil, err
	}
	...
	return &WalletAdaptor{
		...
		etherscanCli: NewEtherscanClient(conf.Fullnode.Ada.TpApiUrl, conf.Fullnode.Ada.TpApiKey),
	}, nil
}

func NewLocalWalletAdaptor(network config.NetWorkType) wallet.WalletAdaptor {
	return newWalletAdaptor(newLocalAdaClient(network))
}
