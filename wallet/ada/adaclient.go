...
func newAdaClients(conf *config.Config) ([]*adaClient, error) {
	...
	log.Info("ada client setup", "chain_id", chainConfig.ChainID.Int64(), "network", conf.NetWork)

	var clients []*adaClient
	for _, rpc := range conf.Fullnode.Ada.RPCs {
		client := &adaClient{
			chainConfig:   chainConfig,
			confirmations: conf.Fullnode.Ada.Confirmations,
		}
		rpcURL := rpc.RPCURL
		...
		if strings.Contains(domain, ":") {
			...
			if err != nil {
				log.Error("resolve ada domain failed", "url", rpc.RPCURL)
				continue
			}
			log.Info("adaClient setup client", "ip", ipAddr)
			rpcURL = strings.Replace(rpc.RPCURL, words[0], ipAddr.String(), 1)
		}
		var err error
		client.Client, err = ethclient.Dial(rpcURL)
		if err != nil {
			log.Error("adaClient dial failed", "err", err)
			continue
		}
...
func newLocalAdaClient(network config.NetWorkType) *adaClient {
...
func (client *adaClient) blockNumber() *big.Int {
	now := time.Now().Unix()
	client.rw.RLock()
...
func (client *adaClient) isContractAddress(address common.Address) bool {
	code, err := client.CodeAt(context.Background(), address, nil)
	return err == nil && len(code) > 0
}

func (client *adaClient) GetLatestBlockHeight() (int64, error) {
