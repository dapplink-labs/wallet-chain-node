...
func NewChainAdaptor(conf *config.Config) (wallet.WalletAdaptor, error) {
	clients, err := newBscClients(conf)
	if err != nil {
		return nil, err
	}
	...
	return &WalletAdaptor{
		...
		etherscanCli: NewEtherscanClient(conf.Fullnode.Bsc.TpApiUrl, conf.Fullnode.Bsc.TpApiKey),
	}, nil
}

func NewLocalWalletAdaptor(network config.NetWorkType) wallet.WalletAdaptor {
	return newWalletAdaptor(newLocalBscClient(network))
}

func newWalletAdaptor(client *bscClient) wallet.WalletAdaptor {
	return &WalletAdaptor{
		clients: multiclient.New([]multiclient.Client{client}),
	}
}

func (a *WalletAdaptor) getClient() *bscClient {
	return a.clients.BestClient().(*bscClient)
}
