...
func newOpClients(conf *config.Config) ([]*opClient, error) {
	...
	var clients []*opClient
	for _, rpc := range conf.Fullnode.Op.RPCs {
		client := &opClient{
			chainConfig:   chainConfig,
			confirmations: conf.Fullnode.Op.Confirmations,
		}
		rpcURL := rpc.RPCURL
...
func newLocalOpClient(network config.NetWorkType) *opClient {
...
func (client *opClient) blockNumber() *big.Int {
	now := time.Now().Unix()
	client.rw.RLock()
...
func (client *opClient) isContractAddress(address common.Address) bool {
	code, err := client.CodeAt(context.Background(), address, nil)
	return err == nil && len(code) > 0
}

func (client *opClient) GetLatestBlockHeight() (int64, error) {
