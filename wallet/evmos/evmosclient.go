...
func newEvmosClients(conf *config.Config) ([]*evmosClient, error) {
	...
	var clients []*evmosClient
	for _, rpc := range conf.Fullnode.Evmos.RPCs {
		client := &evmosClient{
			chainConfig:   chainConfig,
			confirmations: conf.Fullnode.Evmos.Confirmations,
		}
		rpcURL := rpc.RPCURL
...
func newLocalEvmosClient(network config.NetWorkType) *evmosClient {
...
func (client *evmosClient) blockNumber() *big.Int {
	now := time.Now().Unix()
	client.rw.RLock()
...
func (client *evmosClient) isContractAddress(address common.Address) bool {
	code, err := client.CodeAt(context.Background(), address, nil)
	return err == nil && len(code) > 0
}

func (client *evmosClient) GetLatestBlockHeight() (int64, error) {
